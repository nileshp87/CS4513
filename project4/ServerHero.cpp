...
ServerHero::ServerHero() {
  ...
  if (!p_temp_sprite) {
    log_manager.writeLog("ServerHero::ServerHero(): Warning! Sprite '%s' not found", "ship");
  } else {
...
int ServerHero::eventHandler(Event *p_e) {
...
void ServerHero::kbd(EventKeyboard *p_keyboard_event) {
  WorldManager &world_manager = WorldManager::getInstance();
